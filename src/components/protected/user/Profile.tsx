...
import { Box, Button, Heading, Text, VStack, Flex, Spinner, Divider } from '@chakra-ui/react';
import { getLikedReviewsFromUser } from '../../../service/apiclient'; // Update the path as needed
        ...
        if (userId) {
            const fetchLikedReviews = async () => {
                try {
                    ...
                    console.log('Fetched liked reviews:', reviews);
                } catch (error) {
                    ...
                    {loadingLikedReviews ? (
                        ...
                        <VStack spacing={4} align="stretch">
                            {likedReviews.map((likedReview) => (
                                <Box
                                    key={likedReview.Review.id} // Access the Review object for id
                                    p={4}
                                    bg="gray.50"
                                    borderRadius="md"
                                    boxShadow="md"
                                >
                                    <Text fontSize="lg" color="teal.700" fontWeight="bold">
                                        {likedReview.Review.title} {/* Access the Review object for title */}
                                    </Text>
                                </Box>
                            ))}
                        </VStack>
